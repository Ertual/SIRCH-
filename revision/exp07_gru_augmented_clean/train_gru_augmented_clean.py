...
def build_model() -> Model:
    ...
    x = layers.GRU(GRU_UNITS, return_sequences=False)(features)
...
def main() -> None:
    ...
    anti_leak_path = exp04_outputs / "anti_leak_report.json"
    augmented_manifest_path = exp04_outputs / "augmented_train_manifest.csv"
    validation_manifest_path = (
        project_root / "revision" / "exp01_manifests_sha256" / "outputs" / "validation_manifest.csv"
    )
    output_root = Path(
        os.environ.get(
            "SIRCH_EXP07_MODEL_DIR",
    ...
    model_path = output_root / "sirch_model_gru_augmented_clean.h5"
    best_weights_path = output_root / "best_gru_augmented_clean.weights.h5"
    best_state_path = output_root / "best_state.json"
    log_path = output_root / "training_log_gru_augmented_clean.csv"
    status_path = output_root / "training_status.json"

    if status_path.exists() and model_path.exists():
        previous = json.loads(status_path.read_text(encoding="utf-8"))
        if previous.get("status") == "complete":
            raise RuntimeError("L'experience GRU enrichie est deja terminee.")
    ...
    if len(train_rows) != 2818 or len(validation_rows) != 594:
        raise RuntimeError("Comptes attendus: train augmente=2818, validation=594.")
    ...
    model = build_model()
    checkpoint_pattern = checkpoints / "gru_augmented_clean_epoch_{epoch:03d}.weights.h5"
    checkpoint_regex = re.compile(r"gru_augmented_clean_epoch_(\d+)\.weights\.h5$")
    checkpoint_files = sorted(
        checkpoints.glob("gru_augmented_clean_epoch_*.weights.h5"),
    ...
    environment = {
        **base_status,
        ...
        "architecture": "EfficientNetB0 frozen + GRU(256) + Dense(128)",
        "optimizer": "Adam",
    ...
    write_json(output_root / "training_environment.json", environment)
    write_json(
        status_path,
        {
            **base_status,
    ...
    callbacks = [
        ...
        BestStateCallback(best_state_path, best_val_loss),
    ...
    try:
        ...
        if not best_weights_path.exists():
            raise RuntimeError("Les meilleurs poids GRU n'ont pas ete produits.")
        ...
        print(f"Entrainement GRU enrichi termine. Meilleur modele: {model_path}", flush=True)
